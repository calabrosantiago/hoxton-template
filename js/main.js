...
//Disclaimer handler
var disclaimer = getById("background");
var exitDisclaimer = getById("exit_disclaimer");
...
Creative.init = function () {
  Creative.tl
    ...
    .to(
      [imgFrame01],
      { duration: _fadeInSpeed, alpha: 1 },
      "frame01+=" + _fadeOutSpeed
    )
    .to(
      [copyFrame01a, copyFrame01b],
      { duration: _fadeInSpeed, alpha: 1 },
      "frame01+=" + (_fadeOutSpeed + _fadeInSpeed)
    )
    ...
    .to(
      [copyFrame02a, copyFrame02b],
      { duration: _fadeInSpeed, alpha: 1 },
      "frame02+=" + (_fadeOutSpeed + _fadeInSpeed)
    )
    ...
    .to(
      [copyFrame01a, copyFrame01b],
      {
        ...
        alpha: 0,
        overwrite:
          _arrFrameWaits[1] <= _fadeOutSpeed + _fadeInSpeed ? true : "auto",
      },
      "frame01+=" + _arrFrameWaits[1]
    )
    .to(
      [copyFrame02a, copyFrame02b],
      {
      ...
      "frame02+=" + _arrFrameWaits[1]
    )

    .addLabel(
      "end",
      "frame05+=" + (_fadeOutSpeed + _fadeInSpeed + _fadeInSpeed)
    )
...
function showHideBoolElements() {
  ...
  for (var i = 0; i < _arrFrameObjs.length; i++) {
    for (var fIndex = 1; fIndex <= _totalFrames; fIndex++) {
      ...
      var ifSkip = false;
      if (fIndex < _totalFrames) {
        // if skipping a frame we need to not add tweens for it
      ...
      if (_arrFrameObjs[i].arrFrames[fIndex - 1] === true) {
        // if the element should be shown on a frame
        if (_arrFrameObjs[i].ifShowing === false && ifSkip === false) {
          // if not already showing add tween to show it
          Creative.tl.to(
            _arrFrameObjs[i].eleRef,
            { duration: showSpeed, alpha: 1, delay: showDelay },
            tlPos
          );
          _arrFrameObjs[i].ifShowing = true;
        }
      } // if the element should not be shown on a frame
      else {
        if (_arrFrameObjs[i].ifShowing === true && ifSkip === false) {
          // if currently showing add tween to remove it
          Creative.tl.to(
            _arrFrameObjs[i].eleRef,
            { duration: hideSpeed, alpha: 0, delay: 0 },
            tlPos
          );
